...
export class SceneManager {
    ...
    private setupCamera(canvas: HTMLCanvasElement): void {
        this.camera = new ArcRotateCamera("camera", -Math.PI / 2, Math.PI / 3, 40, Vector3.Zero(), this.scene);
    }
    ...
    private updateCamera(): void {
        ...
        if (state === "MAIN_MENU") {
            // Cinematic rotating camera for menu
            this.cameraAngle += 0.002;
            this.camera.alpha = this.cameraAngle;
    ...
    private setupEnvironment(): void {
        ...
        if (positions) {
            for (let i = 0; i < positions.length; i += 3) {
                ...
                if (dist < 15) positions[i + 1] = -2.5; 
                else if (dist < 22) positions[i + 1] = (dist - 22) * 0.35; 
                else positions[i + 1] = Math.sin(x * 0.1) * Math.cos(z * 0.1) * 0.8;
            }
            terrain.setVerticesData("position", positions);
            const normals: number[] = []; VertexData.ComputeNormals(positions, terrain.getIndices() as number[], normals);
        ...
        waterMat.diffuseColor = new Color3(0.1, 0.4, 0.9); waterMat.alpha = 0.5;
        waterMat.specularColor = new Color3(1, 1, 1); waterMat.backFaceCulling = false;
